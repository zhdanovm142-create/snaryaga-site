...
- `scripts/verify-static.mjs` — 25 проверок: SEO (19), целостность ссылок
  (2), perf-маркеры (4). Код возврата 1 при любом FAIL — встраивать в CI/пре-деплой.
...
- Регрессия прошлого: e2e поймал 404 по `leggings-1.jpg` после конверсии —
  поэтому блок B верификатора обязателен к прогону.
...
2. Скрипты >10 строк сохранять в `scripts/` и запускать файлом, не inline;
   править точечно (Edit), не переписывать целиком.
3. Деливери — в `/home/z/my-project/download/`.
4. Код и комментарии — на русском (правило проекта: язык пользователя ru).
5. Не трогать: `out/` руками, `src/components/ui/*` точечно, punycode-домен
   (всегда ASCII в robots/sitemap/canonical/og).
6. Перед любым «готово» по сборке: `bun run build` + verify-static (25/25)
   + браузерный smoke + обновить static-build/ (+ .nojekyll) + worklog.
7. Секреты: в чат/код не вставлять (в истории был скомпрометирован GitHub
