 ...
 *
 * Как добавить видео — два способа:
 *
 * 1. САМЫЙ ПРОСТОЙ: положите .mp4 файл в /public/videos/ — он появится
 *    автоматически (API /api/videos сканирует папку).
 *
 * 2. С ПОДПИСЯМИ: дополнительно заполните массив VIDEO_META ниже — тогда
 *    у видео появится имя автора, роль и название товара. Если файла нет
 *    в VIDEO_META — покажется подпись «Видео-отзыв #N».
 *
...
/**
 * Подписи к видео. Ключ — имя файла.
 * Заполняйте по мере появления видео. Непрописанные файлы получат подпись
 * «Видео-отзыв #N».
 */
const VIDEO_META: VideoMeta[] = [
...
export default function ReviewsCarousel() {
  ...
  // Загружаем список видео из API при монтировании.
  useEffect(() => {
    let cancelled = false;
    fetch("/api/videos")
      .then((r) => r.json())
  ...
  // Сопоставляем обнаруженные файлы с метаданными.
